...
const Restaurants = ({ restaurantList, restaurant_list, history, location }) => {
  const [categories, setCategories] = useState([]);
  const [defaultSearchValue, setDefaultSearchValue] = useState('');
  const [renderRestaurantList, setRenderRestaurantList] = useState(true);
  const [renderCategorizedRestaurants, setRenderCategorizedRestaurants] = useState(false);
  const [renderSearchRestaurants, setRenderSearchRestaurants] = useState(false);
  ...
  useEffect(() => {
    ...
    if (location && location.state) {
      setDefaultSearchValue(location.state);
      handleSearchBar(location.state);
    }
  ...
  const handleCategoriesCheckbox = (event) => {
    ...
    if (updatedCategories.length > 0) {
      setRenderRestaurantList(false);
      setRenderCategorizedRestaurants(true);
      setRenderSearchRestaurants(false);
    } else {
      setRenderRestaurantList(true);
      setRenderCategorizedRestaurants(false);
      setRenderSearchRestaurants(false);
    }
  };
  ...
  const handleSearchBar = (searchText) => {
    ...
    if (searchText.length > 0) {
      setRenderRestaurantList(false);
      setRenderCategorizedRestaurants(false);
      setRenderSearchRestaurants(true);
    } else {
      setRenderRestaurantList(true);
      setRenderCategorizedRestaurants(false);
      setRenderSearchRestaurants(false);
    ...
    setSearchRestaurants(result);
    setDefaultSearchValue(searchText);
  };

  const handleViewMenuBtn = (resDetails) => {
    history.push('/restaurant-details', resDetails);
  };
    ...
    <div className="container bg-white p-3 px-0 mb-4" key={restaurant.id}>
      <div className="row">
        <div className="col-lg-3 col-md-3 col-sm-12 px-0 text-center">
          <img style={{ width: '70%' }} alt={restaurant.userName} src={restaurant.userProfileImageUrl} />
        </div>
        ...
        <div className="col-lg-3 col-md-3 col-sm-12 py-4 px-0">
          ...
          <button
            type="button"
            onClick={() => handleViewMenuBtn(restaurant)}
  ...
  const _renderRestaurantList = () => restaurantList?.map(renderRestaurantCard);

  const _renderCategorizedRestaurants = () => {
    if (!restaurantList) return null;
    return restaurantList
      .filter((res) => res.typeOfFood.some((food) => categories.includes(food)))
      .map(renderRestaurantCard);
  };

  const _renderSearchRestaurants = () => searchRestaurants.map(renderRestaurantCard);
        ...
        <div className="container">
          <div className="row">
            ...
            <div className="col-lg-2 col-md-2 col-sm-12">
              <div className="listing-filter">
                ...
                <ul className="filter-list">
                  {['Apple Juice', 'BB.Q', 'Beef Roast', 'Chicken Roast', 'Cheese Burger', 'Cold Coffee'].map((category) => (
                    <li key={category}>
                      <div className="custom-control custom-checkbox">
                        ...
                        />
                        <label className="custom-control-label" htmlFor={category.toLowerCase().replace(/\s/g, '-')}>
                          {category}
                        </label>
                      </div>
                    </li>
            ...
            <div className="col-lg-7 col-md-7 col-sm-12">
              <h4 className="mb-3">Restaurants Found</h4>
              <div className="container px-0">
                <div className="col-lg-12 col-md-12 col-sm-12 mb-4 px-0">
                  {renderSearchRestaurants && _renderSearchRestaurants()}
                  {renderCategorizedRestaurants && _renderCategorizedRestaurants()}
                  {renderRestaurantList && _renderRestaurantList()}
                </div>
              </div>
            </div>
