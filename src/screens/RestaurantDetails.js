...
const RestaurantDetails = () => {
  ...
  useEffect(() => {
    if (location.state) {
      setResDetails(location.state);
      fetchMenuItems(location.state.id);
    } else {
  ...
  const addToCart = (item) => {
    ...
    setTotalPrice((prev) => prev + Number(item.itemPrice));
  };

  const removeCartItem = (index) => {
    const itemPrice = Number(cartItemsList[index].itemPrice);
    setCartItemsList((prev) => prev.filter((_, i) => i !== index));
  ...
  const handleConfirmOrderBtn = async () => {
    ...
    if (userDetails.isRestaurant) {
      Swal.fire({ title: 'Error', text: 'You are not able to order', icon: 'error' });
      return;
    }

    if (cartItemsList.length === 0) {
      Swal.fire({ title: 'Error', text: 'You have to select at least one item', icon: 'error' });
      return;
    }

    try {
      await orderNow(cartItemsList, totalPrice, resDetails, userDetails, navigate);
      Swal.fire({ title: 'Success', text: 'Successfully Ordered', icon: 'success' }).then(() => {
        navigate('/my-orders');
      });
    } catch (error) {
      ...
      <div className="container border-bottom pb-2 px-lg-0 px-md-0 mb-4" key={item.id}>
        <div className="row">
          <div className="col-lg-2 col-md-3 col-8 offset-2 offset-lg-0 offset-md-0 px-0 mb-3 text-center">
            <img style={{ width: '70px', height: '70px' }} alt={item.itemTitle} src={item.itemImageUrl} />
          </div>
          ...
          <div className="col-lg-3 col-md-3 col-sm-12 px-0 text-center">
            <span className="mx-3">RS.{item.itemPrice}</span>
            <span className="menuItemsListAddBtn" onClick={() => addToCart(item)}>
  ...
  const renderCartItemsList = () =>
    cartItemsList.map((item, index) => (
      <li className="food-item border-bottom pb-2 mb-3" key={index}>
        <div className="row">
          ...
          <div className="col-4 pl-0 text-right">
            <p className="mb-0">
              <span className="food-price">RS.{item.itemPrice}</span>
              <span onClick={() => removeCartItem(index)} className="remove-food-item">
          ...
          <div className="row">
            ...
            <div className="col-lg-2 col-md-2 col-sm-12">
              <div className="listing-category">
                ...
                <ul className="category-list">
                  {['Kebabs', 'Chicken', 'Burgers', 'Biryani', 'Sauces', 'Vegatable Dishes', 'Garlic Bread', 'Specials'].map(
                    (cat) => (
                      <li key={cat}>
                        <p>{cat}</p>
                      </li>
            ...
            <div className="col-lg-7 col-md-7 col-sm-12">
              <div className="container">
                <div className="row">
                  <div
                    className={`col-12 col-lg-4 col-md-4 text-center ${activeTab === 'tab1' ? 'res-details-tab-active' : ''}`}
                    onClick={() => handleTabs('tab1')}
                  >
                    <p className="res-details-tab-text">
                      <FontAwesomeIcon icon="concierge-bell" className="mr-3" />
                      Menu
                    </p>
                  </div>
                  <div
                    className={`col-12 col-lg-4 col-md-4 text-center ${activeTab === 'tab2' ? 'res-details-tab-active' : ''}`}
                  ...
                  <div
                    className={`col-12 col-lg-4 col-md-4 text-center ${activeTab === 'tab3' ? 'res-details-tab-active' : ''}`}
                  ...
                  <div className="row menu-section">
                    <div className="col-12 bg-white p-4">
                      <div className="input-group input-group-sm mb-4 mt-2">
                        <input type="text" className="form-control search-menu-input" placeholder="Search food item" />
                        <div className="input-group-append">
                          <span className="input-group-text search-menu-text" id="search-menu">
                ...
                {activeTab === 'tab3' && (
                  <div className="row info-section">
                    <div className="col-12 bg-white p-4">
                      ...
                      <p>
                        Base prepared fresh daily. Extra toppings are available in choose extra. Choose your sauce: Go for BBQ
                        sauce or piri piri sauce on your pizza base for no extra cost. Choose your cut: Triangular, square, fingers
                        or Un cut on any size pizza
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="container bg-white py-3 order-card">
                ...
                {cartItemsList.length > 0 ? (
                  <>
                    ...
                    <p style={{ backgroundColor: '#f1f3f8', padding: '10px 15px' }}>
                      Total+{' '}
                      <span style={{ float: 'right', color: '#2f313a', fontSize: '14px', fontWeight: 700 }}>
                        <em>RS.{totalPrice}</em>
                      </span>
                    </p>
                  </>
                ) : (
                  <p className="text-success">There are no items in your basket.</p>
                )}
